...
{
	Connection c=DbConnect.getConnection();
	PreparedStatement ps= c.prepareStatement("insert into addProperty values(?,?,?,?,?,?)");
	ps.setInt(1, obj.getPropertyId());
	ps.setInt(2, obj.getBaseValue());
	ps.setInt(3, obj.getBuiltupArea());
	ps.setInt(4, obj.getAge());
	ps.setString(5, obj.getIsCity());
	ps.setDouble(6, obj.getPropertyTax());
